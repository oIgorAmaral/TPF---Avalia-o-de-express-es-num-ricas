...
int Operador(char c) { return c == '+' || c == '-' || c == '*' || c == '/' || c == '%' || c == '^'; }
...
int AssociativoEsquerda(const char *op) { 
    if (strlen(op) == 1 && op[0] == '^') return 0; 
    return 1; 
}
        ...
        if (ehDigito(StrInfixa[k]) || (StrInfixa[k] == '.' && ehDigito(StrInfixa[k+1]))) {
            ...
            while (!pilhaCharVazia(&pilha_op) && topoPilhaChar(&pilha_op) != '(') {
                ...
                if ( (obterPrecedencia(repr_op_topo_pilha) > obterPrecedencia(str_op_temp)) ||
                     (obterPrecedencia(repr_op_topo_pilha) == obterPrecedencia(str_op_temp) && AssociativoEsquerda(repr_op_topo_pilha)) 
                   ) {
...
char *getFormaInFixa(char *StrPosFixa) {
    ...
    while (token != NULL) {
        ...
        } else if (strlen(token) == 1 && Operador(token[0])) {
